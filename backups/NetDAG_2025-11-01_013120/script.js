...
// From sidebar include (.sidenav / .sidenav-links)
 ...
 (function pageNav(){
  ...
  const here = (file && location.pathname.includes('/menu/')) ? 'menu/' + file : file;
  const idx = pages.findIndex(p => p.href === here);
  if (idx === -1) return;

  const prev = pages[idx-1], next = pages[idx+1];
  nav.innerHTML = '';
  if (prev){ const a = document.createElement('a'); a.href = (location.pathname.includes('/menu/') ? '../' : './') + prev.href; a.textContent = '← ' + prev.label; nav.appendChild(a); }
  if (next){ const b = document.createElement('a'); b.href = (location.pathname.includes('/menu/') ? '../' : './') + next.href; b.textContent = next.label + ' →'; nav.appendChild(b); }
})();
